...
Thank you for downloading DS-Tracks - the safe, simple music playback system for community radio stations.

This guide will help you create a ready-to-use DS-Tracks appliance in about 15 minutes.
...
7. [Using DS-Tracks](#7-using-ds-tracks)
...
The Raspberry Pi Imager is a free tool that writes the DS-Tracks system to your SSD.
...
### Step 2.2: Download DS-Tracks Image

Download the DS-Tracks appliance image from where you registered:

| File | Size | Description |
|------|------|-------------|
| `DS-Tracks-v2.0.img.xz` | ~1 GB | The appliance image (compressed) |
...
2. Type: `certutil -hashfile Downloads\DS-Tracks-v2.0.img.xz SHA256`
3. Compare the result with the checksum on the download page

**Mac/Linux:**
1. Open Terminal
2. Type: `sha256sum ~/Downloads/DS-Tracks-v2.0.img.xz`
3. Compare the result with the checksum on the download page
...
### Step 3.4: Choose the DS-Tracks Image

1. Click **CHOOSE OS**
2. Scroll down and select **Use custom**
3. Navigate to your downloaded `DS-Tracks-v2.0.img.xz` file
4. Click **Open**
...
1. Click **NEXT**
2. When asked about OS customisation, click **NO** (DS-Tracks has its own configuration)
...
1. Find the file called `ds-config.txt`
...
<!-- IMAGE: Screenshot showing ds-config.txt open in Notepad -->
...
|---------|-------------|---------|
| `STATION_NAME` | Your station's full name | `My Station` |
| `STATION_SHORT_NAME` | Short name (no spaces) | `DS` |
| `STATION_WEBSITE` | Your website URL | `https://example.com` |
...
**For WiFi:**
- If you configured WiFi in `ds-config.txt`, it will connect automatically

### Step 5.5: Power On

1. Connect the USB-C power supply to the Pi
2. The Pi will boot automatically
3. You'll see the Raspberry Pi logo, then DS-Tracks will load
...
The first time you power on, DS-Tracks performs automatic setup:
...
When setup is complete, you'll see the DS-Tracks login screen with your station name.

<!-- IMAGE: Screenshot of DS-Tracks login screen -->

**Congratulations! Your DS-Tracks appliance is ready to use.**
...
2. **Insert the USB drive** into the DS-Tracks appliance (not the Raspberry Pi itself - use the USB ports on your studio desk if available)
...
See the **DS-Tracks User Guide** for complete instructions on:
- Managing sessions
- Auto-play mode
...
3. Go to: `http://[Pi-IP-Address]/ds-tracks/admin_customize.php`
...
| Screen is rotated wrong | Edit `ds-config.txt` and set `SCREEN_ROTATION` |
...
| Sound from wrong output | In DS-Tracks, check audio output settings |
...
| WiFi won't connect | Check credentials in `ds-config.txt`, try ethernet |

### Starting Fresh

If you need to completely reset DS-Tracks:
...
- **Documentation:** https://yoursite.com/ds-tracks/docs
- **FAQ:** https://yoursite.com/ds-tracks/faq
- **Community Forum:** https://yoursite.com/ds-tracks/forum

### Reporting Issues

If you find a bug or have a feature request:

- **GitHub Issues:** https://github.com/your-repo/ds-tracks/issues
...
- Your DS-Tracks version (shown at bottom of screen)
...
3. Find the device named `ds-[stationname]`
...
If you need a fixed IP address, add to `ds-config.txt`:
