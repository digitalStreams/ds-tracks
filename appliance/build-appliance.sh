#!/bin/bash
# ============================================================
# DS-Tracks Appliance Build Script
# ============================================================
#
# This script configures a fresh Raspberry Pi OS installation
# into a DS-Tracks appliance ready for imaging.
#
# Prerequisites:
# - Fresh Raspberry Pi OS Lite (64-bit) installation
# - Internet connection
# - DS-Tracks source files in /tmp/DS-Tracks/
#
# Usage:
...
# Configuration
# Auto-detect source directory (parent of the appliance/ folder this script lives in)
SCRIPT_DIR="$(cd "$(dirname "$0")" && pwd)"
DS_SOURCE_DIR="$(dirname "$SCRIPT_DIR")"
DS_INSTALL_DIR="/var/www/html/ds-tracks"
APPLIANCE_DIR="$DS_SOURCE_DIR/appliance"
LOG_FILE="/var/log/ds-build.log"
...
check_prerequisites() {
    ...
    if [ ! -d "$DS_SOURCE_DIR" ]; then
        log_error "DS-Tracks source not found at $DS_SOURCE_DIR"
        log_error "Please copy the DS-Tracks folder to /tmp/"
        exit 1
    fi

    if [ ! -f "$DS_SOURCE_DIR/scripts/install-raspberry-pi.sh" ]; then
        log_error "scripts/install-raspberry-pi.sh not found in source directory"
        exit 1
    fi
...
phase2_install_ds_tracks() {
    log_header "Phase 2: Install DS-Tracks"

    cd "$DS_SOURCE_DIR"

    log "Running DS-Tracks installer..."
    chmod +x scripts/install-raspberry-pi.sh
    ./scripts/install-raspberry-pi.sh

    log "Verifying installation..."
    if curl -s -o /dev/null -w "%{http_code}" http://localhost/ds-tracks/ | grep -q "200\|302"; then
        log_success "DS-Tracks installation verified"
    else
        log_warn "Could not verify installation - Apache may need restart"
        systemctl restart apache2
    fi

    log_success "DS-Tracks installed"
}
...
phase3_install_kiosk() {
    ...
    # chromium-browser was renamed to chromium in Bookworm+
...
EOF
    ...
    touch /var/log/ds-kiosk.log
    chown pi:pi /var/log/ds-kiosk.log
...
phase4_install_first_boot() {
    ...
    cp "$APPLIANCE_DIR/first-boot/ds-first-boot.sh" /usr/local/bin/
    chmod +x /usr/local/bin/ds-first-boot.sh

    log "Installing first-boot service..."
    cp "$APPLIANCE_DIR/first-boot/ds-first-boot.service" /etc/systemd/system/
    systemctl daemon-reload
    systemctl enable ds-first-boot.service

    log "Installing configuration template to boot partition..."
    # Try both possible boot partition locations
    if [ -d /boot/firmware ]; then
        cp "$APPLIANCE_DIR/boot-files/ds-config.txt" /boot/firmware/
        touch /boot/firmware/ds-first-boot-pending
    else
        cp "$APPLIANCE_DIR/boot-files/ds-config.txt" /boot/
        touch /boot/ds-first-boot-pending
    fi
...
phase4b_install_usb_system() {
    ...
    mkdir -p /media/ds-usb

    log "Installing udev rules for USB detection..."
    cp "$APPLIANCE_DIR/usb/99-ds-usb.rules" /etc/udev/rules.d/
    chmod 644 /etc/udev/rules.d/99-ds-usb.rules

    log "Installing USB mount/unmount scripts..."
    cp "$APPLIANCE_DIR/usb/ds-usb-mount.sh" /usr/local/bin/
    cp "$APPLIANCE_DIR/usb/ds-usb-unmount.sh" /usr/local/bin/
    chmod +x /usr/local/bin/ds-usb-mount.sh
    chmod +x /usr/local/bin/ds-usb-unmount.sh
    ...
    echo "www-data ALL=(ALL) NOPASSWD: /bin/umount /media/ds-usb" > /etc/sudoers.d/ds-usb-eject
    chmod 440 /etc/sudoers.d/ds-usb-eject

    log "Reloading udev rules..."
    udevadm control --reload-rules

    log "Creating USB log file..."
    touch /var/log/ds-usb.log
    chmod 644 /var/log/ds-usb.log
...
phase4c_configure_music_drive() {
    ...
    mkdir -p /mnt/ds-music

    # Add fstab entry (nofail = won't block boot if drive is absent)
    if ! grep -q "LABEL=DS-MUSIC" /etc/fstab; then
        echo "" >> /etc/fstab
        echo "# DS-Tracks - Music storage drive (USB SSD labelled DS-MUSIC)" >> /etc/fstab
        echo "LABEL=DS-MUSIC  /mnt/ds-music  auto  defaults,nofail,x-systemd.device-timeout=10  0  2" >> /etc/fstab
    fi
    ...
    echo "www-data ALL=(ALL) NOPASSWD: /usr/local/bin/apply-storage-mode.sh" > /etc/sudoers.d/ds-storage-mode
    chmod 440 /etc/sudoers.d/ds-storage-mode
    ...
    if [ ! -d "$DS_INSTALL_DIR/music" ] && [ ! -L "$DS_INSTALL_DIR/music" ]; then
        mkdir -p "$DS_INSTALL_DIR/music"
    fi
    chown www-data:www-data "$DS_INSTALL_DIR/music" 2>/dev/null || true
...
phase6_configure_display() {
    ...
    if ! grep -q "disable_overscan=1" "$boot_config"; then
        echo "" >> "$boot_config"
        echo "# DS-Tracks display configuration" >> "$boot_config"
...
phase7_optimize() {
    ...
    log "Removing unnecessary packages (appliance doesn't need these)..."

    # Bluetooth — not used on a kiosk appliance
    apt purge -y bluez pi-bluetooth || true

    # WiFi — appliance uses ethernet only
    apt purge -y wpasupplicant firmware-atheros firmware-brcm80211 \
        firmware-libertas firmware-realtek || true

    # Network services not needed on a kiosk
    apt purge -y avahi-daemon nfs-common || true

    # Hotkey daemon — no physical keyboard in kiosk mode
    apt purge -y triggerhappy || true

    # Documentation — not needed on a headless appliance
    apt purge -y man-db manpages info || true

    # Editors — vim is already installed, don't need duplicates
    apt purge -y nano ed || true
    ...
    log "Disabling any remaining unnecessary services..."
...
show_summary() {
    ...
    echo ""
    echo -e "${GREEN}============================================================${NC}"
    echo -e "${GREEN} DS-Tracks Appliance Build Complete!${NC}"
    echo -e "${GREEN}============================================================${NC}"
    echo ""
    echo "The system is now configured as a DS-Tracks appliance."
    echo ""
    echo "Music storage is controlled by MUSIC_STORAGE in ds-config.txt:"
    echo ""
    ...
    echo "  3. This formats and labels it as DS-MUSIC"
    echo ""
    ...
    echo "  6. Save as: DS-Tracks-Master.img"
...
main() {
    log_header "DS-Tracks Appliance Build Script v2.0"
